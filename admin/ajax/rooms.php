<?php
    ...
    if(isset($_POST['add_room']))
    {
        ...
        {
            $flag = 0;
            die('Không thể chuẩn bị truy vấn - Thêm');
        }
    ...
    if(isset($_POST['get_all_rooms']))
    {
        ...
        while($row = mysqli_fetch_assoc($res))
        {
            if($row['status']==1)
            {
                $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-dark btn-sm shadow-none'>Còn phòng</button>";
            }
            else
            {
                $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-warning btn-sm shadow-none'>Hết phòng</button>";
            }
